...
local charset = {
  ...
  ["字母月份"] = { first = 0x3200, last = 0x32FF },
  ["兼容性"] = { first = 0x3300, last = 0x33FF },
  ["扩展A"] = { first = 0x3400, last = 0x4DBF },
  ...
  ["附表文补"] = { first = 0x1F200, last = 0x1F2FF },
  ...
  ["扩展B"] = { first = 0x20000, last = 0x2A6DF },
  ["扩展C"] = { first = 0x2A700, last = 0x2B73F },
  ["扩展D"] = { first = 0x2B740, last = 0x2B81F },
  ["扩展E"] = { first = 0x2B820, last = 0x2CEAF },
  ["扩展F"] = { first = 0x2CEB0, last = 0x2EBEF },
  ["兼表文补"] = { first = 0x2F800, last = 0x2FA1F },
  ["扩展G"] = { first = 0x30000, last = 0x3134F },
  ["扩展H"] = { first = 0x31350, last = 0x323AF },
  ["扩展I"] = { first = 0x2EBF0, last = 0x2EE5D },
...
local function charset_cjk_filter_plus(input, env)
  ...
  if (c_f_p_s) then
    for cand in input:iter() do
      ...
      if (not exists(is_cjk_ext, cand.text)) then      -- 如果當前候選項 `cand` 不含 CJK 擴展漢字

        yield(cand)        -- 結果中仍保留此候選
      end
        -- 上述條件不滿足時，當前的候選 `cand` 沒有被 yield。因此過濾結果中將不含有該候選。
    end
  else
    for cand in input:iter() do
      yield(cand)
...
local function charset_comment_filter(input,env)
  ...
  for cand in input:iter() do  -- 使用 `iter()` 遍历所有输入候选项
    if b then
     for s, r in pairs(charset) do     -- 判断当前候选内容 `cand.text` 中文字属哪个字符集
       if (exists(is_charset(s), cand.text)) then
         cand:get_genuine().comment = cand.comment .. " " .. s            --[[ 修改候选的注释 `cand.comment`因复杂类型候选项的注释不能被直接修改，因此使用 `get_genuine()` 得到其对应真实的候选项--]]
         break
       end--if
      end--for
     end--if
      yield(cand)       -- 在结果中对应产生一个带注释的候选
   end
end

--[[ 本例中定義了兩個 filter，故使用一個表將兩者導出
  return { filter = charset_cjk_filter, comment_filter = charset_comment_filter }--]]
