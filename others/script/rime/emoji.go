...
// 检查 emoji_others.txt 里的中文映射是否存在于 base 词库中
func checkOthersTXT() {
	// 打开文件
	file, err := os.Open(filepath.Join(RimeDir, "opencc/emoji_others.txt"))
	if err != nil {
		log.Fatalln(err)
	}
	defer file.Close()
	...
	// base+dicts_cn/others.dict.yaml
	othersDictYamlSet := mapset.NewSet[string]()
	othersDictYaml, err := os.Open(filepath.Join(RimeDir, "dicts_cn/others.dict.yaml"))
	if err != nil {
		log.Fatalln(err)
	}
	...
	// 检查： emoji-map.txt 中的映射是否存在于 base+dicts_cn/others.dict.yaml 词库中，有差集即不存在
	...
	for _, text := range set.Difference(dictSet).ToSlice() {
		...
		fmt.Println("❌ emoji_others.txt 与 base 的差集：", text)
	}
}
