...
func main() {
	...
	if len(os.Args) > 1 {
		...
		case "t":
			...
			return
		case "tp":
			rime.Pinyin(filepath.Join(rime.RimeDir, "dicts_cn/temp.txt"))
			return
		}
	}
...
SORT:
	...
	rime.Sort(filepath.Join(rime.RimeDir, "dicts_cn/41448.dict.yaml"), 2)
	rime.Sort(rime.BasePath, 3)
	rime.Sort(rime.ExtPath, 3)
	rime.Sort(rime.TencentPath, 4)
	rime.Sort(filepath.Join(rime.RimeDir, "dicts_en/en.dict.yaml"), 2)
}
